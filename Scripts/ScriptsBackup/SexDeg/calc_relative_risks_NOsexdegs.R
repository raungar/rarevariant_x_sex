...
# infile="/Volumes/groups/smontgom/raungar/Sex/Output/sexdeg_v8/Outliers/outliersTOP_noglobal_varAnnot_medz_zthresh2.5_nphen3_x_f_beta0.111_cadd15_x.txt.gz"
...
exp_data_NOsexdeg<-exp_data%>% dplyr::filter(AbsBetaSex==0 | AbsBetaSex==-Inf | AbsBetaSex==Inf) #AbsBetaSex #was AbsOutlierBetaSex
exp_outliers = exp_data_NOsexdeg %>% dplyr::filter(Y=="outlier") #was just expdata
exp_outliers_over = exp_data_NOsexdeg %>% dplyr::filter(Y=="outlier") %>% dplyr::filter(MedZ>0)#was just expdata
exp_outliers_under = exp_data_NOsexdeg %>% dplyr::filter(Y=="outlier") %>% dplyr::filter(MedZ<0)#was just expdata
exp_controls = exp_data_NOsexdeg %>% dplyr::filter(Y=="control") #was jsut expdata
...
for(exp_type in exp_types){
  if(exp_type=="all"){
    ...
    for (vcat in (vcats)) {
      ...
      #any
        ...
        outliers_passed = this_exp_outliers %>% dplyr::filter(genetype == vcat)%>% dplyr::filter(AbsOutlierBetaSex==0 | AbsOutlierBetaSex==-Inf | AbsOutlierBetaSex==Inf)
        ...
        # exp_yy = nrow(this_exp_outliers %>% dplyr::filter(genetype == vcat) %>% dplyr::filter( AbsOutlierBetaSex==0 | AbsOutlierBetaSex==-Inf | AbsOutlierBetaSex==Inf) %>% dplyr::filter(has_variant == "rare")) # %>% dplyr::filter(cadd_phred>cadd_min))
      ...
      this_risk=data.frame(Risk = unlist(riskvals)[1],
                           ...
                           outlier_direction=exp_type,Type = 'NoSexDEG',row.names=NULL)
  ...
  #DO NO FILTER VARCAT
      ...
      outliers_passed = this_exp_outliers %>% dplyr::filter(AbsOutlierBetaSex==0 | AbsOutlierBetaSex==-Inf | AbsOutlierBetaSex==Inf)
      exp_yn=nrow( outliers_passed   %>% dplyr::filter(has_variant != "rare"))  #%>% dplyr::filter(cadd_phred>cadd_min))
      exp_yy=nrow( outliers_passed   %>% dplyr::filter(has_variant == "rare"))  #%>% dplyr:s:filter(cadd_phred>cadd_min))
