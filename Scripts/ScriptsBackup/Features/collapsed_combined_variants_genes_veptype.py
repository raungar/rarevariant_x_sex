...
outfile=args.out
filtervar_preswap=args.vartype
cadd_min=float(args.cadd_min)
print("arguments parsed.")

filtervar=filtervar_preswap.replace("-","_")
...
with gzip.open(combined_file,"r") as f_read:
	for line in f_read.readlines():
		...
		if(int(geno) == 0):
			continue
		store_line=[chrom,start,end,ensg, vartype,ind,sex,var_location, maf_gtex, maf_gnomad,maf_use,genetype,cadd_raw,cadd_phred,geno,filtervar,vepconsq]
		print(ensg)
