...
parser.add_argument("-d","--prefix",help="RARE DIR PATH, no trailing /")
...
parser.add_argument("-o","--out_all",help="path for gathered expression, include gz ending")
parser.add_argument("-a","--out_aut",help="path for gathered expression, include gz ending")
parser.add_argument("-x","--out_x",help="path for gathered expression, include gz ending")
parser.add_argument("-s","--suffix",help="suffix")
parser.add_argument("-b","--gtf_file_aut",help="path for this gtf file autosomal")
parser.add_argument("-f","--gtf_file_x",help="path for this gtf filex")
...
exprdir = args.prefix #os.environ["RAREDIR"]
...
gtf_file_aut=args.gtf_file_aut
gtf_file_x=args.gtf_file_x
out_all=args.out_all
out_aut=args.out_aut
out_x=args.out_x
my_group=args.group
suffix=args.suffix
...
with open(gtf_file_x,"r") as f:
	 gene_list_x = [str(line.split("\t")[0]) for line in f]
with open(gtf_file_aut,"r") as f:
	 gene_list_aut = [str(line.split("\t")[0]) for line in f]
gene_list_both=gene_list_aut+gene_list_x
...
out_x = open(out_x, 'w')
out_aut = open(out_aut, 'w')
out_all.write('\t'.join(['Tissue','Gene'] + individuals) + '\n')
out_x.write('\t'.join(['Tissue','Gene'] + individuals) + '\n')
out_aut.write('\t'.join(['Tissue','Gene'] + individuals) + '\n')
#.decode('utf-8'))
# process tissues one at a time 
print("BEGINNING TO PROCESS")

for tissue in tissues:
	#counts-Lung-m-protect_sex-preprocessing_v8-both-binary.txt
	# filename = exprdir + "/"+tissue + '.' + my_group+ '.peer.ztrans.txt'
	filename = exprdir + tissue + suffix
	print(filename)
	...
	if(1==1):
		...
		filtered_x=np.isin(padded[:,1] , gene_list_x)
		filtered_aut=np.isin(padded[:,1] , gene_list_aut)
		filtered_both=np.isin(padded[:,1] , gene_list_both)
		padded_x = padded[filtered_x]
		padded_aut = padded[filtered_aut]
		padded_both = padded[filtered_both]

		np.savetxt(out_all, padded_both,encoding='utf-8', fmt='%s', delimiter='\t', newline='\n')
		np.savetxt(out_aut, padded_aut,encoding='utf-8', fmt='%s', delimiter='\t', newline='\n')
		np.savetxt(out_x, padded_x,encoding='utf-8', fmt='%s', delimiter='\t', newline='\n')
	#except:
	#	print(tissue)
