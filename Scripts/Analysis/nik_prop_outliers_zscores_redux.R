...
prop_outliers <- function(group_diff = 1, zscore = 2.5, sd_1 = 1, sd_2 = 1, w_1 = 0.5, stratify_direction = T){
  ...
  integ_change <- integ_gained + integ_lost
  ...
  if(stratify_direction){
    return(c(lost = sum(integ_gained), gained = sum(integ_lost), net = -sum(integ_net),change = sum(integ_change))  )
  } else {
    return(c(lost = sum(integ_lost), gained = sum(integ_gained), net = sum(integ_net),change = sum(integ_change))  )
  }
...
zscore <- 2.5
means_prop_gained_lost <- data.frame(t(sapply(delta_mus, 
                                        function(dmu) prop_outliers(group_diff = dmu, zscore = zscore, sd_1 = 1, sd_2 = 1, w_1 = 0.5))))
df_means=cbind(delta_mus,means_prop_gained_lost)
melted_df_means=melt(df_means,id.vars="delta_mus")
ggplot(melted_df_means,aes(x=delta_mus,y=value,color=variable))+
  geom_line(position=position_dodge(width=0.05))+theme_bw()+
  xlab("change in mean")


empirical_obs <- rbeta(1E4, 1, 6) * diff(range(delta_mus)) + delta_mus[1]
d <- means_prop_gained_lost

par(mar = c(4,7,17,4), xpd = T)
sds <- 0:1000/100
prop_gained_lost_vars <- data.frame(t(sapply(sds, 
                                        function(this_sd) prop_outliers(group_diff = 0, zscore = zscore, sd_1 = 1, sd_2 = this_sd, w_1 = 0.5,stratify_direction = T))))
df_vars=cbind(sds=sds,prop_gained_lost_vars)
melted_df_vars=melt(df_vars,id.vars="sds")
ggplot(melted_df_vars,aes(x=sds,y=value,color=variable))+
  geom_line()+
  ylab("proportion")+
  #geom_line(position=position_dodge(width=0.05))+
  theme_bw()+
  scale_x_continuous(trans="log10") +
  xlab("change in standard deviation")
