...
pcs=${6}
md=${7}
factors_type=${8} #factors or factors prefix for when transform by sex
logfile=${9} #log file change
incl_sex=${10}  # T or F
sex_contin=${11}
traitsFileName=${12}
...
#do
    ...
    outfile=${prefix}.${sex}.peer.v8ciseQTL.ztrans.txt
   ...
   echo $outfile
	...
	#3 and 4 removed
    Rscript ${scriptdir}/calculate_PEER_residuals_wEQTL.R $traitsFileName ${pcs} \
            ${indir} ${gtex_v8_eqtl_dir}/${tissue}.v8.egenes.txt.gz ${eqtl_genos} \
