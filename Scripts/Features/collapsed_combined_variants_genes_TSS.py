...
with gzip.open(combined_file,"r") as f_read:
	for line in f_read.readlines():
		...
		TSS=line_split[14].strip()
		TSE=line_split[15].strip()
		if(cadd_phred=="NA"):
			cadd_phred=0
		#if homo dom, dont consider
		if(int(geno) == 0):
			continue
		store_line=[chrom,start,end,ensg, vartype,ind,sex, maf_gtex, maf_gnomad,maf_use,genetype,cadd_raw,cadd_phred,geno,TSS,TSE]
		...
		if ensg in this_inds_dic[ind]:
			...
			dic_current_min_TSSdistance=(this_inds_dic[ind][ensg])[14]

			print(ensg,"-- thismaf/min maf: ",maf_use,"/",dic_current_min_maf, 
				"   and thiscadd/mincadd ",cadd_phred,"/",dic_current_min_cadd_phred,
				"   and thisTSS/minTSS ",TSS,"/",dic_current_min_TSSdistance)
			#check first that this line passes the min cadd threshold (since will be sent to common eventually anyway)
			#and make sure th
			if((abs(int(TSS))<int(dic_current_min_TSSdistance)) and (float(maf_use) < tss_maf_min)):
