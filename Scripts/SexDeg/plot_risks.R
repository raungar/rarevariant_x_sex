...
mydir="/Volumes/groups/smontgom/raungar/Sex/Output/sexdeg_v8redo/RR"
...
ggplot(to_plot,aes(x=as.factor(max_maf),y=Risk,group=as.factor(outlier_direction),fill=sex, alpha=as.factor(outlier_direction))) +#, 
  ...
  ggtitle(paste0("SEXDEG RR for sex: ", unique(to_plot$sex),", z=", unique(to_plot$z),", chr=",unique(to_plot$chr),", method=",unique(to_plot$outlierType),
                ", cadd=",unique(to_plot$cadd), " beta=", unique(to_plot$beta_val)))+
  ...
  geom_text(size=4, aes(group=outlier_direction,label=ifelse((Pval)<0.05,round(Risk,1),"NS")), position = position_dodge(width = 1),vjust = -0.5) +
...
to_plot=tissue_risks%>%dplyr::filter(Cat=="all")%>%filter(cadd==15) %>%filter(max_maf==0.001)%>% filter(outlierType=="outliersTOP") %>%  #dplyr::filter(chr=="x")
 dplyr::filter(outlier_direction=="all") # filter(sexdeg_type=="any")#%>% 
