...
ztrans.tissue = function(tissue, orig.dir, out.dir, covs, read.filt = 6, tpm.filt = 0.1,tpm_dif_file=F,inds) {
    print("in ztrans")
    tpm = fread(paste0(orig.dir, tissue, '.tpm.txt'))
    reads = fread(paste0(orig.dir, tissue, '.reads.txt'))
    ...
    if(length(sex_table)==1){
        ...
        write.table(tpm.out.single, paste0(out.dir, tissue, ".log2.ztrans.",this_sex,".txt"), quote = F, sep = '\t', row.names = T, col.names = T)
      ...
      #keep 
      if(min(sex_table)==max(sex_table)){
        min_sex=1
        max_sex=2
      }else{
      ...
      if(min_sex==1){
        ...
        covs.m<-covs.m.all[covs.m.all %in% colnames(tpm)]
	      half_m<-floor(length(covs.m)/2)
	      covs.m.half<-names(rev(sort(inds[covs.m]))[1:half_m])
	      covs.f.all<-covs$SUBJID[covs$SEX==max_sex]
	      covs.f.sub<-covs.f.all[covs.f.all %in% colnames(tpm)]
	      # half_f<-floor(length(covs.f.sub)/2)
        #sample to individuals that are in the most tissues
        covs.f<-names(rev(sort(inds[covs.f.sub]))[1:length(covs.m)]) 
        covs.f.half<-names(rev(sort(inds[covs.f.sub]))[1:half_m]) 
      }else if(min_sex==2){
        min_sex_fm="female"
      ...
      print(paste0(tissue, " is male=",length(covs.m)," and female=",length(covs.f)))
      ...
      write.table(tpm.out.m, paste0(out.dir, tissue, '.log2.ztrans.m.txt'), quote = F, sep = '\t', row.names = T, col.names = T)
      ...
      write.table(tpm.out.f, paste0(out.dir, tissue, '.log2.ztrans.f.txt'), quote = F, sep = '\t', row.names = T, col.names = T)
      
      #Both
      print(paste0("WRITING BOTH:",out.dir, tissue, '.log2.ztrans.both.txt'))
      ...
      write.table(tpm.out.both, paste0(out.dir, tissue, '.log2.ztrans.both.txt'), quote = F, sep = '\t', row.names = T, col.names = T)


      #Both half
      print(paste0("WRITING BOTH HALF:",out.dir, tissue, '.log2.ztrans.both_half.txt'))
      ...
      write.table(tpm.out.both.half, paste0(out.dir, tissue, '.log2.ztrans.both_half.txt'), quote = F, sep = '\t', row.names = T, col.names = T)
...
peer.orig.dir = paste0(dir, '/preprocessing_v8/PEER_v8/')
# peer.dir = paste0(dir, '/preprocessing_v8/PEER_v8/')
peer.null.dir = paste0(dir, '/nullshuffled_v8/PEER_v8/')
...
# subject.file = "/Volumes/groups/smontgom/raungar/Sex/Output//nullshuffled_v8/sample_sex_scrambled.txt"
# sample.file = "/Volumes/groups/smontgom/shared/GTEx/all_data/GTEx_Analysis_2017-06-05_v8/sample_annotations/GTEx_Analysis_2017-06-05_v8_Annotations_SampleAttributesDS.txt"
# map_file_prefix = "/Volumes/groups/smontgom/raungar/Sex/Output/preprocessing_v8/gtex_2017-06-05_v8_samples_tissues.txt"
...
system(paste('mkdir -p', peer.null.dir))
...
pcs = pcs[, -c(1,2)]
sex = read.csv(subject.file, header = F, stringsAsFactors = F, sep = '\t')
print(head(sex))
colnames(sex)<-c("SUBJID","SEX")
...
if(do_tissues){
  ...
  ind_table<-get_ind(tissues, peer.orig.dir,euro)
	sapply(tissues, ztrans.tissue, orig.dir=peer.orig.dir,out.dir=peer.null.dir, covs = covariates,
