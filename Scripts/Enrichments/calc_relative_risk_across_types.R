...
for (vcat in (vcats)) {
   print(vcat)
    ...
    exp_yn = nrow(exp_outliers %>% dplyr::filter(variant_cat == vcat) %>% dplyr::filter(has_variant != "rare") %>% dplyr:filter(cadd_phred>cadd_min))
    exp_yy = nrow(exp_outliers %>% dplyr::filter(variant_cat == vcat) %>% dplyr::filter(has_variant == "rare") %>% dplyr:filter(cadd_phred>cadd_min))
   #exp_nn = nrow(exp_outliers %>% dplyr::filter(variant_cat == vcat) %>% dplyr::filter( has_variant == "rare"))
   #exp_ny = nrow(exp_outliers %>% dplyr::filter(variant_cat == vcat) %>% dplyr::filter(has_variant != "rare"))
 ...
 exp_yn_all=nrow(dplyr::filter(exp_outliers,has_variant != "rare") %>% dplyr:filter(cadd_phred>cadd_min))
 exp_yy_all=nrow(dplyr::filter(exp_outliers,has_variant == "rare") %>% dplyr:filter(cadd_phred>cadd_min))
#exp_nn_all = nrow(exp_outliers %>% dplyr::filter( has_variant == "rare"))
#exp_ny_all = nrow(exp_outliers %>% dplyr::filter(has_variant != "rare"))
...
risks = rbind(risks, data.frame(Risk = err_all$tab[2,5],
                                ...
                                exp_yn=exp_yn_all,
                                exp_yy=exp_yy_all,
                                num_outliers=nrow(exp_outliers),
                                sex=sex,z=zscore,nphen=nphen,
                                Type = 'Total expression'))   
