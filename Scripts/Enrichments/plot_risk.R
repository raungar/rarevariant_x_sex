...
to_plot=all_risks_p%>%dplyr::filter(CATEGORY=="all")%>%filter(outlierType=="outliers") %>%
  filter(maxmaf=="0.001")%>%dplyr::filter(chr=="x")
...
#
 ggplot(to_plot,aes(x=maxmaf,group=sex,fill=sex)) + 
   geom_crossbar(aes(x=maxmaf,y=Risk,ymin=Lower,ymax=Upper), position=position_dodge(width=1)) +
   ...
   ggtitle(paste0("Relative Risk: (outlierType=",unique(to_plot$outlierType),
                  ",nphen=",unique(to_plot$nphen),",MAF=",unique(to_plot$maxmaf),"), num outlier range [",
   ...
   geom_text(aes(group=interaction(real_or_null,sex),x=as.character(chr),y=Risk,label=paste0("n=",num_outliers,"\nRR=",round(Risk,1),"\np=",round(padj,2))),  position = position_dodge(width = 1), vjust=-1.25) +
   geom_hline(yintercept=1,color="red")
 
 
 
 to_plot=all_risks_p%>%dplyr::filter(CATEGORY=="all")%>%filter(outlierType=="outliers") %>% mutate(is_sig=ifelse(Pval<0.01,T,F)) %>%
   filter(cadd==0)  %>% dplyr::filter(nphen==5)%>% dplyr::filter(sex=="both")  %>% dplyr::filter(z==2.5) #%>%filter(maxmaf==0.001) # %>% filter(chr=="x")
 ggplot(to_plot,aes(x=(maxmaf),y=Risk,group=chr,color=chr,shape=chr,label=num_outliers)) + 
      theme(axis.text.x = element_text(angle = 45,  hjust=1))+
...
####outliers plotting
  ...
  ggplot(to_plot,aes(x=as.factor(exp_type),y=outliers_tested, group=sex, #linetype
  ...
  ggplot(to_plot,aes(x=interaction(sex),y=outliers_tested,group=sex,
    ...
    facet_wrap(~as.factor(chr),ncol=3) +xlab("sex") +ylab("Number of Outliers")
